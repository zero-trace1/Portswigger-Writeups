...
![Lab overview - not solved yet](lab3overview.png)
...
1. **Identify the Injection Point**
   ...
   ```
   ![Intercepted GET request in Burp Proxy](lab3-getreq.png)
   The `category` parameter is the obvious candidate — it drives a `WHERE` clause on the server, making it a classic
   injection point.
...
3. **Determine the Number of Columns**
   ...
   ```
   ![Testing column count with UNION SELECT in Repeater](lab3-trial1.png)
...
4. **Inject the Version Query**
   ...
   ```
   ![Querying v$version banner column](lab3-trial2.png)
...
5. **Confirm Success**
   ...
   ![Copying the crafted URL from Repeater](lab3-url.png)
